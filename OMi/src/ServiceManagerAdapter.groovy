...
public class ServiceManagerAdapter {
	...
	/**
	 Length of title
	 */
	private final int titleLength = 256;

	/**
	 Length of description
	 */
	private final int descriptionLength = 15;

	/*
	 String trimmer for suitable length
	 Default cutting is 250 symbols long
	 */
	/**
	 * @param toCut
	 * @param len
	 * @return
	 */
	private String fieldCutter(String toCut, int len){
		String result = "";
		if (m_log.isDebugEnabled()){
			m_log.debug("Try to cut with fieldCutter(String toCut, int len, String toPrepend)");
			m_log.debug(toCut + ":::" + len );
		}
		if (toCut != null) {
			if (toCut.length() > len) {
				result = toCut.substring(0,len);
			} else {
				result = toCut;
			}
		}
	...
	/**
	 * @param toCut String, which we will cut
	 */
	private String fieldCutter(String toCut) {
		return fieldCutter(toCut, titleLength);
	}
		...
		try {
			...
			if ((response != null) && (response.length() >= 0)) {
				...
				if (incident != null && incident.name().equals(INCIDENT_TAG)) {
					...
					// Old native description string without cutting 
	 ...
	 */
	private String toExternalEvent(final OprEvent event) {
	...
	private String toExternalEvent(final OprEvent event,
		...
		if (!default_flag) {
			...
			String description = fieldCutter(event.description.trim());
			...
			xmlns: "${INCIDENT_XML_NAMESPACE}") {
				...
				&& (isNewIncident || syncAllOPRPropertiesToSM || SyncOPRPropertiesToSM.contains("description")))
					builder."${DESCRIPTION_TAG}"(fieldCutter(description,descriptionLength))
				...
				&& (syncAllOPRPropertiesToSMActivityLog || SyncOPRPropertiesToSMActivityLog.contains("description"))) {
					...
					activityLog.append( event.description.substring(0, descriptionLength) )
					activityLog.append('\n')
				}
		...
		if (default_flag) {
			...
			if (title && (title.length() > 256 || title.contains('\n'))) {
				...
				if (title.contains('\n'))
					title = title.split('\n')[0].trim()
				//TODO rewrite fieldCutter
				title = fieldCutter(title, titleLength);
				if (m_log.isDebugEnabled()){
					m_log.debug("After a fieldCutter we have: " + title);
				}
			...
			if (description == null) {
				...
				description = fieldCutter(description, descriptionLength);
			}
			if (m_log.isDebugEnabled()){
				m_log.debug("After a fieldCutter we have: " + description);
			}
...
//TODO activity log description 
				...
				&& (syncAllOPRPropertiesToSMActivityLog || SyncOPRPropertiesToSMActivityLog.contains("description"))) {
					...
					if (!isNewIncident)
						activityLog.append(ACTIVITY_LOG_DESCRIPTION_CHANGE)
					activityLog.append(fieldCutter(event.description.trim(),descriptionLength))
					activityLog.append('\n')
				}
