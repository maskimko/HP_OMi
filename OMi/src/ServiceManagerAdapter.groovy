...
public class ServiceManagerAdapter {
  ...
  private static final Set SyncOPRPropertiesToSMActivityLog = ["title", "description", "state", "severity", "priority",
      "annotation", "duplicate_count", "cause", "symptom", "assigned_user", "assigned_group", "astl_operational_device"]
  ...
  /**
	Length of title
  */
  private final int titleLenght = 256;
  ...
  */
  private void fieldCutter(String toCut, int len){
	if (toCut != null) {
		if (toCut.length() <= len) {
			toCut.trim().substing(0,len);
		}
	} else {
		toCut = "";
	}
  }
  private void fieldCutter(String toCut) {
	fieldCutter(toCut, titleLength);
  }
  private void fieldCutter(String toCut, int len, String append) {
	if (append != null) {
		if (toCut == null) {
			toCut = "";
		}
		fieldCutter(toCut + append, len);
	}else {
		fieldCutter(toCut, len);
	}
  }
...
//##################################### DEFAULT SECTION #####################################
	if (default_flag) {
		...
		if (title && (title.length() > 256 || title.contains('\n'))) {
		  ...
		  if (title.contains('\n'))
			title = title.split('\n')[0].trim()
		  fieldCutter(title, titleLength);
		  
		} 
		
		if (description == null) {
			fieldCutter(description, descriptionLength, title);
		} else {
			fieldCutter(description, descriptionLength);
		}
